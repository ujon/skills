...
Usage:
    ...
    note.save("./연구노트_2026-03-13.hwpx")

CLI:
    python research_note_builder.py --format hwpx --title "과제명" --author "기록자" -o ./sample.hwpx
"""
...
class ResearchNoteBuilder:
    ...
    VALID_FORMATS = ("hwpx", "markdown", "docx", "pdf", "docx_spec", "pdf_spec")
    ...
    def _build_docx(self, path: str) -> str:
        from docx_builder import DocxNoteBuilder

        doc = DocxNoteBuilder(font_name=self._font_name, font_size=self._font_size)

        # 머리글/바닥글
        과제명 = self._hf_opts.get("과제명", self._metadata.get("과제명", ""))
        기관명 = self._hf_opts.get("기관명", "")
        if self._hf_mode in (1, 2):  # both or header only
            doc.set_doc_header(과제명=과제명, 기관명=기관명)
        if self._hf_mode in (1, 3):  # both or footer only
            doc.set_doc_footer(과제명=과제명, 기관명=기관명)

        # 제목 블록
        doc.add_header_block("연 구 노 트", self._metadata.get("과제명", ""))

        # 메타데이터 테이블
        meta_rows = self._build_meta_rows()
        doc.add_metadata_table(meta_rows)

        # 본문 섹션
        for i, (title, content) in enumerate(self._sections, 1):
            doc.add_section(i, title, content)
        ...
        if self._data_attachments:
            ...
            run = p.add_run(f"{section_num}. 첨부 데이터")
            run.font.size = Pt(12)
            run.font.bold = True
            doc.doc.add_paragraph()
            for att in self._data_attachments:
                self._render_attachment_docx(doc, att)

        # 서명란
        if self._signature:
            날짜 = self._metadata.get("작성일자", "____-__-__")
            doc.add_signature_block(
                기록자=self._signature.get("기록자", ""),
                점검자=self._signature.get("점검자", ""),
                날짜=날짜,
            )

        return doc.save(path)

    def _render_attachment_docx(self, doc, att: dict):
        """첨부 데이터를 DOCX 요소로 렌더링."""
        fmt = att.get("format", "summary")
        summary = att.get("summary", "")

        if fmt == "table" and isinstance(att.get("data"), list):
            doc.add_data_table(att["data"], summary=summary)
        elif fmt == "code_block" and isinstance(att.get("data"), str):
            doc.add_code_block(att["data"], summary=summary)
        elif fmt == "image_ref" and isinstance(att.get("data"), dict):
            doc.add_image_ref(att["data"])
        else:
            if summary:
                p = doc.doc.add_paragraph()
                p.add_run(f"[{summary}]")
            if att.get("data"):
                ...
                p2.add_run(str(att["data"]))

    # ── PDF 빌더 ──

    def _build_pdf(self, path: str) -> str:
        from pdf_builder import PdfNoteBuilder

        doc = PdfNoteBuilder(font_name=self._font_name, font_size=self._font_size)

        # 머리글/바닥글 (페이지 추가 전에 설정)
        과제명 = self._hf_opts.get("과제명", self._metadata.get("과제명", ""))
        기관명 = self._hf_opts.get("기관명", "")
        if self._hf_mode in (1, 2):
            doc.set_doc_header(과제명=과제명, 기관명=기관명)
        if self._hf_mode in (1, 3):
            doc.set_doc_footer(과제명=과제명, 기관명=기관명)
        ...
        if self._data_attachments:
            section_num = len(self._sections) + 1
            doc.pdf.set_font(doc._font, "B", 12)
            doc.pdf.cell(0, 8, f"{section_num}. 첨부 데이터", new_x="LMARGIN", new_y="NEXT")
            doc.pdf.ln(2)
            for att in self._data_attachments:
                self._render_attachment_pdf(doc, att)
    ...
    def _render_attachment_pdf(self, doc, att: dict):
        """첨부 데이터를 PDF 요소로 렌더링."""
        fmt = att.get("format", "summary")
        summary = att.get("summary", "")
        ...
        if fmt == "table" and isinstance(att.get("data"), list):
            ...
            if summary:
                doc._set_font("")
                doc.pdf.cell(0, 7, f"[{summary}]", new_x="LMARGIN", new_y="NEXT")
            if att.get("data"):
                doc._set_font("")
                doc.pdf.multi_cell(0, 6, str(att["data"]), new_x="LMARGIN", new_y="NEXT")
...
if __name__ == "__main__":
    ...
    if not args.output:
        ...
        args.output = f"./연구노트_{today}{ext[args.format]}"
